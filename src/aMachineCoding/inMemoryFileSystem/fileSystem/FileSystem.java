package aMachineCoding.inMemoryFileSystem.fileSystem;

import aMachineCoding.inMemoryFileSystem.CompositePattern.Directory;
import aMachineCoding.inMemoryFileSystem.CompositePattern.File;
import aMachineCoding.inMemoryFileSystem.CompositePattern.FileSystemNode;

public class FileSystem {

    private final FileSystemNode root; // Root of the virtual file system
    ...
    public boolean createPath(String path) {
        ...
        for (int i = 1; i < pathComponents.length - 1; i++) {
            ...
            if (child.isFile()) {
                return false; // Cannot navigate through a file
            }
        ...
        if (lastComponent.isEmpty() || current.hasChild(lastComponent)) {
            return false; // Duplicate or invalid name
        }
    ...
    public boolean setFileContent(String path, String content) {
        ...
        if (node == null || !node.isFile()) {
            return false; // Must be a valid file
        }
    ...
    public String getFileContent(String path) {
        ...
        if (node == null || !node.isFile()) {
            return null; // Must be a valid file
        }
    ...
    private String getParentPath(String path) {
        // Extract the parent path by trimming the last segment
